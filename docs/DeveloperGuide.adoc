...
Stores a `String` that represents the name of the case. Provides utility methods to:
* check the validity of the string to be stored
* retrieve the string that is stored
* obtain the hash code of the `CaseName` object
...
Stores a `String` that represents the start date of the case. Provides utility methods to:
* check the validity of the string to be stored
* retrieve the string that is stored
* obtain the hash code of the `StartDate` object
...
Stores a `String` that represents the status of the case. The class only stores strings of value "open" or "close", with the default value set to "open" when the constructor is called. Provides utility methods to:
* toggle the status of the case
* retrieve the string that is stored
* obtain the hash code of the `Status` object
...
Newly created cases are stored in the same file as the investigators, in xml format. `XmlAdaptedCrimeCases` is used to convert a `CrimeCase` object, or `String` parameters describing a `CrimeCase` object into an `XmlAdaptedCrimeCase` object for JAXB use (creating the XML).
...
Note that the investigator field uses an `XmlAdaptedPerson` object, which means that the the complete information from the `Person` object is duplicated within the `cases` tags in the XML file.

+

Below is an example of how the cases and investigators are stored in the file:
    ...
    <investigapptor>
        ...
        <cases>
            ...
            <startDate>14/02/13</startDate>
            <status>open</status>
            <tagged>Murder</tagged>
        </cases>
        <cases>
            <name>Project New</name>
            <description>New project description</description>
            <investigator>
            ...
            <startDate>23/11/2013</startDate>
            <status>open</status>
            <tagged>Homicide</tagged>
            <tagged>Fraud</tagged>
        </cases>
        <persons>
            <name>Alex Yeoh</name>
...
|`* * *` |investigator |find certain tag(s) |view who are under a certain team/category
