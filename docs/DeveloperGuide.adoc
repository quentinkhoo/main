...
The `Investigator` package is a extended from the `Person` package, allowing it to handle the basic functions of a `Person` package while
having additional properties required for an investigator. The additional properties are:
...
The `CrimeCase` package consists of multiple classes used to contain information about a case.

==== `CaseName` class +

Stores a `String` that represents the name of the case. Provides utility methods to check the validity of the string to be stored, retrieve the string that is stored, and obtain the hash code of the `CaseName` object.
...
Stores a `String` that represents the description of the case. Provides utility methods to:
* check the validity of the string to be stored
* retrieve the string that is stored
* obtain the hash code of the `Description` object
...
Stores a `String` that represents the start date of the case. Provides utility methods to check the validity of the string to be stored, retrieve the string that is stored, and obtain the hash code of the `StartDate` object.
...
Stores a `String` that represents the status of the case. The class only stores strings of value "open" or "close", with the default value set to "open" when the constructor is called. Provides utility methods to toggle the status of the case, retrieve the string that is stored, and obtain the hash code of the `Status` object.
...
Stores `CaseName`, `Description`, `Person`, `StartDate`, `Status`, and `UniqueTagList` objects to represent information of the case. Provides utility methods to retrieve the different objects, delete a tag, and obtain the hash code of the `CrimeCase` object.

   /**
     * Constructs a {@code CrimeCase}.
     * Every field must be present and not null
     */
    public CrimeCase(CaseName name, Description description, Person currentInvestigator, StartDate startDate, Status status, Set<Tag> tags)
...
* `AddCaseCommandParser`

`AddCaseCommandParser` parses the given `String` of arguments in the context of the `AddCaseCommand` and returns an `AddCaseCommand` object for execution. A `ParseException` is thrown if the user input does not conform to the expected format.
...
The `AddCaseCommand` that extends the `UndoableCommand` class is implemented with two overloaded constructors, either of which can be used depending on convenience:
...
Since `AddCaseCommand` extends `UndoableCommand`, after the constructor is called, the `preprocessUndoableCommand` and `executeUndoableCommand` methods are executed consecutively right after.

If the `AddCaseCommand(CrimeCase crimeCase)` constructor is used, the logic in the `preprocessUndoableCommand` method is not executed.

Note that the `AddCaseCommand(CaseName name, Description description, Index investigatorIndex, StartDate startDate, Set<Tag> tagList)` constructor above takes in an `Index` object as a parameter, which refers to the index number of the investigator as displayed in `PersonListPanel`.

The `Person` object is retrieved based on the index and the created `CrimeCase` object is stored in `toAdd`:
...
The `executeUndoableCommand` then adds the `CrimeCase` object to the model and throws a `DuplicateCrimeCaseException` if there exists an identical object:
...
* `DeleteCaseCommandParser`

`DeleteCaseCommandParser` parses the given `String` of arguments in the context of the `DeleteCaseCommand` and returns an `DeleteCaseCommand` object for execution. A `ParseException` is thrown if the user input does not conform to the expected format.
...
`preprocessUndoableCommand` first retrieves the `CrimeCase` object based on the `Index` given. The `DeleteCaseCommand` will then be executed to remove the case from the application and update the model. `EventsCenter` reacts to the `SwapTabEvent`, which switches the tab to show the list of cases. The following segment shows the execution code.
...
* `FindCaseCommandParser`

`FindCaseCommandParser` parses the given `String` of arguments in the context of the `FindCaseCommand` and returns an `FindCaseCommand` object for execution. A `ParseException` is thrown if the user input does not conform to the expected format.
...
The `FindCaseCommand` will then be executed and the observable list filteredCrimeCaseList will be updated with the cases whose case name matches the predicate keywords. `EventsCenter` reacts to the `SwapTabEvent`, which switches the tab to show the filtered list of cases. The following segment shows the execution code.
...
* *Marcus Chen*
** *Major enhancement*: Implement suspects and witness into investigapptor
*** This feature allows investigator to easily view suspects and witness related to the cases they are investigating
...
* *Poh Kai Jun*
** *Major Enhancement*: Support for calendar and graph
*** This feature allows an overview of the chronological flow of each case handover and number of cases handled by each investigator
** *Minor Enhancement*: Implement `findtags` command(s)
*** This feature helps the users to filter out investigators/cases that are in a certain team/category according
