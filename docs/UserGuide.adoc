...
You can refer to the screenshots below for an example of how the Investigapptor
would look like after you've executed the `listinvestigatorcases` command.

[[lic-before]]
.Before executing `listinvestigatorcases 1` command
image::userguide-images/lic-before.png[width="800"]
...
want to view by using the index as shown in <<investigatorcase-1>> bellow

[[investigatorcase-1]]
.Using listinvestigatorcases command
image::userguide-images/lic-command1.png[width="900"]

[[investigatorcase-1]]
.Result
image::userguide-images/lic-command2.png[width="900"]
...
We have plan to implement the feature to allow investigator to create and add witnesses to the cases in the investigapptor,
 allowing investigators to track witnesses using the Investigapptor.

==== Adding a suspect `addsuspect` [Coming in v2.0]
We plan to allow investigator to create and add suspects to the cases in the investigapptor, allowing investigators
to track and view suspects involvement in any past or current cases using the Investigapptor.

==== Adding evidence files `addevidence` [Coming in v2.0]
We plan to allow investigator to create and add evidence to cases in the investigapptor, allowing investigators
to view more information about the evidence related in the case.

==== Setting up chain of custody `buildcoc` [Coming in v2.0]
We plan to store the history of investigators in charge of each case, allowing the Investigapptor to display
when the case was transferred and whom the cases was transfer to.

==== Entering commands in other languages `language` [Coming in v2.0]
We plan to add more languages support such as Mandarin and Malay to the Investigapptor in the future to allow non-english
user to be able to use Investigapptor
...
We plan to add the feature which allows Investigapptor to create a PDF file which contains the information
of a case or investigator.

// tag::dataencryption[]
==== Encrypting data files `encryptdata` [Coming in v2.0]

We plan to add data encryption to Investigapptor in the future, which will encrypt and protect the data stored
in investigapptor.
// end::dataencryption[]
