...
Registers an investigator to the application so that it cases can be assigned to him/her.
...
Simply key in the information in the above format in the input bar and press enter. Refer to `Figure 3` as an example
[.left]
.Keying in the command
image::AddInvestigator1.png[width="790"]

Outcome: After pressing enter, the new investigator will appear on the investigator list and a success message will be displayed at the bottom
[.left]
.Outcome
...
Outcome: The success message should appear and the investigator will no long be on the list.
...
==== Finding cases by name: `findcase` [Since v1.3]
