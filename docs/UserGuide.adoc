...
== Quick Start
In order to start using Investigapptor, it is best to follow the instructions below to setup your computer to run Investigapptor
...
In investigapptor, there are several components as shown below in <<Interface>> which you can view and interact with
...
.  *Panel List:* A panel that display the investigators or crime cases. You can toggle between the two by using the
toggle button at the top.

. *Detail Panel:* The display panel will the full information of selected investigator or crime case.

. *Calendar:* The calendar will show when the cases were open and closed.

. *Command Box:* The command box is where you key in the commands to navigate the investigapptor.

. *Display Box:* The display box will display the result to any command you input.

=== Using commands
As the using commands is the main method of interacting with investigapptor, learning how to use the command is important.
