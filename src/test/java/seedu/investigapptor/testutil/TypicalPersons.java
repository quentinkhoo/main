...
public class TypicalPersons {
    ...
    // Manually added
    public static final Person HOON = new InvestigatorBuilder().withName("Hoon Meier").withPhone("8482424")
            .withEmail("stefan@example.com").withAddress("little india").build();
    public static final Person IDA = new InvestigatorBuilder().withName("Ida Mueller").withPhone("8482131")
            .withEmail("hans@example.com").withAddress("chicago ave").build();

    // Manually added - Person's details found in {@code CommandTestUtil}
    public static final Person AMY = new InvestigatorBuilder().withName(VALID_NAME_AMY).withPhone(VALID_PHONE_AMY)
            .withEmail(VALID_EMAIL_AMY).withAddress(VALID_ADDRESS_AMY).withTags(VALID_TAG_FRIEND).build();
    public static final Person BOB = new InvestigatorBuilder().withName(VALID_NAME_BOB).withPhone(VALID_PHONE_BOB)
    ...
    public static Investigapptor getTypicalInvestigapptor() {
        ...
        for (Investigator investigator : getTypicalInvestigtors()) {
            try {
                ia.addInvestigator(investigator);
    ...
    public static List<Investigator> getTypicalInvestigtors() {
